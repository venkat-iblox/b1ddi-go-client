...
func TestClient(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			&ConvertDomainNameConvertParams{
				DomainName: "domain_name",
				Context:    context.TODO(),
			},
			http.Request{
				URL:    &url.URL{Path: "/api/ddi/v1/dns/convert_domain_name/domain_name"},
	...
	for _, tc := range testCases {
		t.Run(tc.testMethodName, func(t *testing.T) {
			...
			c := initConvertDomainNameTestClient(s.URL)
...
func initConvertDomainNameTestClient(url string) ClientService {
