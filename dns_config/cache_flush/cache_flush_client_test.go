...
func TestClient(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			http.Request{
				URL:    &url.URL{Path: "/api/ddi/v1/dns/cache_flush"},
				Method: http.MethodPost,
				Body:   io.NopCloser(strings.NewReader("{\"ophid\":\"oph_id\"}\n")),
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.testMethodName, func(t *testing.T) {
			...
			c := initCacheFlushTestClient(s.URL)
...
func initCacheFlushTestClient(url string) ClientService {
